...
- **Mode colours** (vibranti per sfondo chiaro): LitClock `#2563EB`, SlowMovie `#D97706`, Wikipedia `#DC2626`, Weather `#059669`, Museo `#7C3AED`, Koan `#4F46E5`, Cucina `#EA580C`, Flora `#16A34A`, APOD `#0EA5E9`, Mars `#C2410C`.
...
- 10 modes in registry: `litclock`, `slowmovie`, `wikipedia`, `weather`, `museo`, `koan`, `cucina`, `flora`, `apod`, `mars` (all active). ANSi and Teletext removed.
...
- **Web dashboard**: React SPA (Vite + Tailwind) at `http://<DEVICE_IP>:<PORT>`; Vibemilk Default theme (warm milk-white bg `#F0EDE8`, magenta accent `#FF1DA5`, navy text `#1A1A2E`); Lobster logotype; Funnel Display variable for interface text; CRT RenderingOverlay (NoiseCanvas + scanlines + magenta sweep); Stats card; PlaylistEditor with DnD reorder; Language selector (full names, always visible, auto-saves); Weather settings (location search + units toggle); rotating taglines; Koan archive wall page (dark gallery, masonry grid, search, export ZIP). Netmilk logo in footer.
...
- **APOD mode**: random historical NASA APOD image (1995-06-16 to yesterday), seeded deterministically per refresh interval. Sidebar layout: 500px left (image, PIL-enhanced, `ImageOps.fit`) + 300px right (pure black `(0,0,0)` panel). Panel: `APOD · date` header (Inconsolata 20pt white / 17pt grey, one line), title (Lobster 26pt, max 3 lines), explanation body, horizontal separator, copyright (Inconsolata 14pt, truncated with `…`), clock footer (Inconsolata 17pt). Video days: uses `thumbnail_url`; if absent → title-only fallback. API key: `apod_api_key` in `secrets.yaml` (optional, falls back to DEMO_KEY with 30 req/h limit). Cache: `cache/apod_data.json` (keyed by `_cached_for` date field), `cache/apod_image.jpg`, `cache/apod_image_date.txt`. Refresh: 300 s (clock updates; image cached by date — network call once per unique date). Error screen: Lobster 38pt title, Taviraj 22pt subtitle, classified error reason (HTTP code, timeout, DNS, network), footer with API key type and timeout value. Timeout: 45s (Pi Zero W needs extra margin over WiFi).
- **Mars mode**: Curiosity rover raw photos from JPL API (`mars.nasa.gov/api/v1/raw_image_items/?order=sol+desc&per_page=500`). No API key. Camera preference ranking: MAST_RIGHT → MAST_LEFT → MAHLI → CHEMCAM_RMI → NAV_LEFT_B → NAV_RIGHT_B → FHAZ/RHAZ. Photo selected by `hour % len(photos_sorted)` for intra-day rotation. Full-bleed layout (`ImageOps.fit` 800×480). Overlay band: `band_h=135`, `fill=(0,0,0,255)` (pure black, fully opaque). 4 lines: (1) `CURIOSITY · {camera_full}` in Signika-Bold 28pt white; (2) `Sol N · Received on Earth: DD Month YYYY` in Taviraj-Italic 19pt amber `(240,175,110,255)`; (3) `Photo #ID · N total transmissions from Curiosity` in Taviraj-Italic 19pt warm brown `(190,140,100,255)`; (4) `HH:MM · D Month YYYY` in Inconsolata 16pt grey. `_camera_full_name()` strips `"Sol N: "` prefix from JPL `title` field. `_fmt_date()` handles both `"YYYY-MM-DD"` and `"YYYY-MM-DDTHH:MM:SS.sssZ"`. Cache: `cache/mars_photos.json` (keyed by `_date` field, daily), `cache/mars_image.jpg`, `cache/mars_image_id.txt`. Refresh: 3600 s. Note: JPL `?mission=mars2020` filter is silently ignored — always returns Curiosity. Error screen: Signika-Bold 36pt title, Taviraj 22pt subtitle, classified error reason, footer with API info and timeout.
...
- **Welcome screen**: TV frame graphic (`assets/img/talevision-frame.png`, RGBA composited on white) boot splash, shown for **30 s**. Lobster 75pt BLACK title, random tagline (20 options), `— STARTING IN 30 SECONDS —` in red, compact BBS info box (HOSTNAME.local/IP/DASHBOARD only, `inner_w=64`), `TaleVision v3.0 · Netmilk Studio` footer in blue. Saved to `cache/welcome_frame.png` on every boot. Rendered by `talevision/render/welcome_screen.py`, triggered from `orchestrator.run()`.
...
- **APOD/Mars error screens are diagnostic**: error screens classify the failure type (HTTP status, timeout, DNS, network unreachable) and show the reason on-screen with large fonts, plus footer with API key type and configured timeout. Errors are stored in `_last_error` instance variable, set by `_fetch_apod_data()`/`_fetch_latest_photos()` and `_fetch_image()`. Helper functions `_classify_http_error()` and `_classify_url_error()` (APOD) / `_classify_mars_error()` (Mars) parse `urllib.error.HTTPError`, `urllib.error.URLError`, `socket.timeout`, `socket.gaierror` into user-facing messages.
- **Pi Zero W API timeouts**: the Pi's slow WiFi + CPU means API calls can timeout more often than expected. APOD timeout set to 45s (not 30s). If "unavailable" screens appear frequently, increase the timeout or check Pi WiFi signal strength.
...
- **3 providers** (round-robin rotation): Met (NYC, ~200k, no key), Cleveland (~41k, no key), V&A (London, ~732k, no key). All free APIs, no auth required. Total pool: ~973k public-domain artworks.
