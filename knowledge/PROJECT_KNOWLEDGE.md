...
- **Mode accent colours**: LitClock `#6A9FBF`, SlowMovie `#E8A838`, Wikipedia `#D06B50`, Weather `#7FA87F`, Museo `#B8860B`, Koan `#8B7D6B`, Cucina `#D2691E`.
...
- **Koan archive page**: dedicated full-page view with CSS columns masonry grid, search by theme/pen name/words, "load more" pagination (30/batch), Export ZIP button. Accessible via "view all →" link in dashboard.
...
- **Web dashboard**: React SPA (Vite + Tailwind) at `http://<DEVICE_IP>:<PORT>`; Solar Dust theme (bg `#1A1410`, gold `#E8A838`, terracotta `#D06B50`, cream text `#F0E6D6`); Lobster logotype; Chakra Petch interface font; CRT vintage RenderingOverlay (NoiseCanvas + TuningGauge SVG + scanlines); Stats card; PlaylistEditor with DnD reorder; Language selector (full names, always visible, auto-saves); rotating taglines; Koan archive wall page (masonry grid + search + export ZIP). Netmilk logo in footer.
