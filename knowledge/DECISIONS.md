...
## 2026-04-01 - APOD/Mars: Diagnostic Error Screens and Timeout Increase

- Context: APOD mode showed "APOD unavailable / the stars are quiet today" with tiny fonts (Lobster 26pt, Taviraj 16pt) and zero diagnostic info. Mars showed "signal lost" similarly. On the Pi Zero W, the actual failure was `The read operation timed out` — the 30s timeout was insufficient for the Pi's slow WiFi. The generic error messages made it impossible to diagnose without SSH-ing into the Pi and reading journalctl logs.
- Decision: (1) Error screens redesigned with larger fonts (Lobster/Signika 36-38pt title, Taviraj 22pt subtitle, Inconsolata 15pt hints). (2) Fetch methods now classify errors specifically: HTTP status codes (403 invalid key, 429 rate limit, 5xx server error), socket.timeout, socket.gaierror (DNS), network unreachable. Error reason displayed on screen with word-wrap. (3) Footer shows API key type (DEMO_KEY vs custom) and configured timeout. (4) APOD timeout increased from 30s to 45s. (5) Helper functions `_classify_http_error()`, `_classify_url_error()` (APOD) and `_classify_mars_error()` (Mars) parse urllib exceptions into user-facing messages.
- Impact/Tradeoffs: Error screens now self-diagnose — users can see "Connection timed out", "DNS lookup failed", "Invalid API key" etc. directly on the e-ink display without needing SSH access. The 45s timeout adds 15s to worst-case render time when the API is unreachable, but prevents false failures on the Pi's slow WiFi.
